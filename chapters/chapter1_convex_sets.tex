...
\chaptertitle{Convex Sets}
\section{Convexity}
\subsection{Cone}
\begin{definition}[Cone]
A set $K \in \R^n$, when $x \in K $ implies $\alpha x \in K$.
\end{definition}
A non convex cone can be hyper-plane~\autocite{aliPowderBedFusion2022}.\\
...
\end{example}
For a convex set $x \in C \Rightarrow x = \sum \alpha_i x_i$~\autocite{aminComputationalHomogenizationElastic2018}. 
...
\end{definition}
Interpretation: There is a convex quadratic $\frac{1}{2}u || \cdot ||^2$ that lower bounds f~\autocite{aminiMLCPMProcessMonitoring2018}.
\begin{example}
